...
public class ModifyStringResourcesMojo extends AbstractMojo {
  ...
  private void modify(byte[] content) throws Exception {
    ...
    replace(content, "ProductVersion\0xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\0",
            "ProductVersion\0" + version + "\0");
  }
  ...
  private void replace(byte[] content, String searchStr, String replaceStr) throws Exception {
    ...
    if (index >= 0) {
      ...
      throw new Exception(
              "Could not replace '" + searchStr + "' with '" + replaceStr + "'. Original string not found.");
    }
  }
